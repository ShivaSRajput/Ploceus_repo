...
resource "azurerm_virtual_machine_extension" "virtual_machine_extension" {
  ...
  auto_upgrade_minor_version = each.value.type_handler_version == "*" ? each.value.auto_upgrade_minor_version : null
  automatic_upgrade_enabled  = each.value.type == "CustomScript" ? each.value.automatic_upgrade_enabled : false
  settings                   = <<SETTINGS
                                      {
