locals {
  host_pool_pooled   = { for k, v in var.desktop_host_pool_variables : k => v if lookup(v, "host_pool_type", null) == "Pooled" }
  host_pool_personal = { for k, v in var.desktop_host_pool_variables : k => v if lookup(v, "host_pool_type", null) == "Personal" }
}
