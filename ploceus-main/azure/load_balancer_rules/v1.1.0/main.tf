...
resource "azurerm_lb_rule" "load_balancer_rule" {
  ...
  probe_id                       = each.value.load_balancer_rule_probe_name == null ? null : "/subscriptions/${each.value.load_balancer_subscription_id}/resourceGroups/${each.value.load_balancer_resource_group_name}/providers/Microsoft.Network/loadBalancers/${each.value.load_balancer_name}/probes/${each.value.load_balancer_rule_probe_name}"
