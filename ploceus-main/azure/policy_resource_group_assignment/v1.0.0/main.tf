locals {
  is_user_identity_exists = { for k, v in var.resource_group_policy_variables : k => v if lookup(v, "is_user_identity_import_needed", false) == true }
}

data "azurerm_resource_group" "rg_name" {
  for_each = var.resource_group_policy_variables
...
resource "azurerm_resource_group_policy_assignment" "rg_policy" {
  ...
  resource_group_id    = data.azurerm_resource_group.rg_name[each.key].id
