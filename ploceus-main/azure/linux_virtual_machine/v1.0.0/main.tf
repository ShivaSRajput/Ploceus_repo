#Linux Virtual Mchine
locals {
  is_public_ip_exists           = { for k, v in var.linux_vm_nic_variables : k => v if lookup(v, "is_public_required", false) == true }
  is_application_gateway_exists = { for k, v in var.linux_virtual_machine_variables : k => v if lookup(v, "is_gateway_load_balancer_required_to_be_attached", false) == true }
  is_disk_encryption_set_exists = { for k, v in var.linux_virtual_machine_variables : k => v if lookup(v, "is_disk_encryption_set_required", false) == true }
  is_boot_diagnostics_exists    = { for k, v in var.linux_virtual_machine_variables : k => v if lookup(v, "is_boot_diagnostics_required", false) == true }
  is_user_identity_required     = { for k, v in var.linux_virtual_machine_variables : k => v if lookup(v, "identity_type", false) != "SystemAssigned" }
  generate_ssh_keys             = { for k, v in var.linux_virtual_machine_variables : k => v if(lookup(v, "generate_new_ssh_key", false) == true) }
  use_existing_ssh_keys         = { for k, v in var.linux_virtual_machine_variables : k => v if(lookup(v, "use_ssh_keys", false) == true && lookup(v, "generate_new_ssh_key", true) == false) }
}

data "azurerm_subnet" "subnet_id" {
  for_each             = var.linux_vm_nic_variables
...
resource "azurerm_linux_virtual_machine" "linux_virtual_machine" {
  ...
  disable_password_authentication = each.value.disable_password_authentication == null ? true : each.value.disable_password_authentication
  ...
  patch_mode                 = each.value.patch_mode == null ? "ImageDefault" : each.value.patch_mode
  ...
  dynamic "admin_ssh_key" {
    for_each = length(local.generate_ssh_keys) == 0 ? (length(local.use_existing_ssh_keys) == 0 ? {} : local.use_existing_ssh_keys) : local.generate_ssh_keys
    content {
      public_key = length(local.generate_ssh_keys) == 0 ? data.azurerm_key_vault_secret.existing_ssh_keys_key_valut_secret[each.key].value : tls_private_key.generate_ssh_keys_tls_private_key[each.key].public_key_openssh
