...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  ...
  default_node_pool {
    ...
    availability_zones     = each.value.default_node_pool_availability_zones
  ...
  addon_profile {
    open_service_mesh {
      enabled = each.value.open_service_mesh_enabled
    }
    ingress_application_gateway {
      enabled    = each.value.ingress_application_gateway_enabled
      gateway_id = each.value.ingress_application_gateway_enabled == false ? null : data.azurerm_application_gateway.application_gateway[each.key].id
    }
    kube_dashboard {
      enabled = each.value.kube_dashboard_enabled
    }
    oms_agent {
      enabled                    = each.value.oms_agent_enabled
