locals {
  is_user_identity_exists = { for k, v in var.management_group_policy_variables : k => v if lookup(v, "is_user_identity_import_needed", false) == true }
}

data "azurerm_management_group" "management_group_id" {
  for_each     = var.management_group_policy_variables
...
resource "azurerm_management_group_policy_assignment" "management_group_policy" {
  for_each             = var.management_group_policy_variables
  ...
  management_group_id  = each.value.is_management_group_exist == false ? data.azurerm_management_group.management_group_id[each.key].id : each.value.management_group_id
